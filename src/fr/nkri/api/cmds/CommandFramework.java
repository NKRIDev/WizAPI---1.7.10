...
import fr.nkri.api.cmds.bukkit.BukkitCommand;
import fr.nkri.api.cmds.bukkit.BukkitCompleter;
import fr.nkri.api.cmds.interfaces.Command;
import fr.nkri.api.cmds.interfaces.Completer;
...
public class CommandFramework implements CommandExecutor {
    ...
    public CommandFramework(Plugin plugin) {
        ...
        if(plugin.getServer().getPluginManager() instanceof SimplePluginManager) {
            ...
            try {
                ...
                fr.nkri.api.logs.Logs.sendLog("[WizAPI]", "Unable to access commandMap.", fr.nkri.api.logs.enums.LogsType.ERROR);
    ...
    public boolean handleCommand(final CommandSender sender, final org.bukkit.command.Command cmd, final String label, final String[] args) {
        for(int i = args.length; i >= 0; i--) {
            ...
            if(commandMap.containsKey(cmdLabel)) {
                ...
                final fr.nkri.api.cmds.interfaces.Command command = method.getAnnotation(fr.nkri.api.cmds.interfaces.Command.class);
    ...
    public void registerCommands(final Object obj) {
        for(Method m : obj.getClass().getMethods()) {
            if(m.getAnnotation(fr.nkri.api.cmds.interfaces.Command.class) != null) {
                final fr.nkri.api.cmds.interfaces.Command command = m.getAnnotation(fr.nkri.api.cmds.interfaces.Command.class);
            ...
            else if(m.getAnnotation(fr.nkri.api.cmds.interfaces.Completer.class) != null) {
                final fr.nkri.api.cmds.interfaces.Completer comp = m.getAnnotation(Completer.class);
    ...
    public void registerCommand(final Command command, final String label, final Method m, final Object obj) {
        ...
        if(map.getCommand(cmdLabel) == null) {
            org.bukkit.command.Command cmd = new fr.nkri.api.cmds.bukkit.BukkitCommand(cmdLabel, this, plugin);
    ...
    public void registerCompleter(final String label, final Method m, final Object obj) {
        ...
        if(map.getCommand(cmdLabel) == null) {
            org.bukkit.command.Command command = new fr.nkri.api.cmds.bukkit.BukkitCommand(cmdLabel, this, plugin);
            map.register(plugin.getName(), command);
        }

        if(map.getCommand(cmdLabel) instanceof fr.nkri.api.cmds.bukkit.BukkitCommand) {
            fr.nkri.api.cmds.bukkit.BukkitCommand command = (BukkitCommand) map.getCommand(cmdLabel);
            if (command.completer == null) {
                command.completer = new fr.nkri.api.cmds.bukkit.BukkitCompleter();
            }
            ...
            try {
                ...
                if(field.get(command) == null) {
                    field.set(command, new fr.nkri.api.cmds.bukkit.BukkitCompleter());
                }

                if(field.get(command) instanceof fr.nkri.api.cmds.bukkit.BukkitCompleter) {
